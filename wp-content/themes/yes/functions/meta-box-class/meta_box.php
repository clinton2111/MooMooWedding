...
function custom_meta_box_field($field, $meta = null, $repeatable = null) {
    ...
    switch ($type) {
        ...
        default:
            ...
            break;
        case 'date':
            echo '<input type="old_' . $type . '" name="' . esc_attr($name) . '" id="' . esc_attr($id) . '" value="' . esc_attr($meta) . '" class="date"/>
                    <br />' . $desc;
            break;
        case 'url':
        ...
        case 'radio':
            ...
            break;
        case 'map':
            echo '<input name="' . esc_attr($name) . '" class="regular-text" type="hidden" id="' . esc_attr($id) . '" value="' . esc_attr($meta) . '" />';
            echo '<div id="gmap_loc" style="width: 100%;height: 250px;"></div>';
            echo '<label class="lat">Latitude: <strong>0</strong>, </label>
            <label class="lng">Longitude: <strong>0</strong>, </label>
            <label class="zoom">Zoom level: <strong>0</strong></label>
            <label class="address">Address: <strong>0</strong></label>';
            break;
    } //end switch
}
...
class Custom_Add_Meta_Box {
    ...
    function admin_enqueue_scripts() {
        global $pagenow;
        // In first array, we check if we are adding or editing post and in second we are checking if post type equals the one called in class
        if (in_array($pagenow, array('post-new.php', 'post.php')) && in_array(get_post_type(), $this->page)) {
            // js
           // wp_enqueue_script( 'jquery-ui-datepicker' );
            wp_enqueue_style( 'jquery-ui-style', 'http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.1/themes/smoothness/jquery-ui.css', true);
            wp_enqueue_script('meta_box_script', get_template_directory_uri() .'/functions/meta-box-class/js/custom.js', array('jquery-ui-datepicker'));
            // css
            wp_enqueue_style('meta_box', get_template_directory_uri() .'/functions/meta-box-class/css/meta_box.css');
        }
    }
